...
(module more-scheme '#%kernel
  (#%require '#%paramz racket/private/define
             (only "bootstrap-lang.ss" let else) "library.ss" "list.ss"
             (for-syntax '#%kernel racket/private/stx racket/private/small-scheme racket/private/stxcase-scheme racket/private/qqstx))

  (define-syntax when
    (lambda (stx)
      (syntax-case stx ()
        ([_ expr body ...]
         (syntax (if expr (begin body ...) (void)))))))

  (define-syntax unless
    (lambda (stx)
      (syntax-case stx ()
        ([_ expr body ...]
         (syntax (if expr (void) (begin body ...)))))))
